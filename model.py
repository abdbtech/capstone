...
# combine vulnerability predictors with simple addition and create intensity rate.
# Multiplicative was chose for amplification effect of casualties on a vulnerable population
# Intensity = casualty rate (per 1000) * 1 + (vulnerability_rate /100)
noaa_census["vulnerability_rate"] = noaa_census["PRED12_PE"] + noaa_census["PRED3_PE"]
noaa_census["intensity"] = noaa_census["casualty_rate"] * (
    1 + (noaa_census["vulnerability_rate"] / 100)
)
...
if REBUILD:
    ...
    try:
        ...
        dbt.print_status("Creating spatial table with corrected coordinates...")
        ...
        dbt.print_status("✓ Spatial table created: disaster_risk_counties_spatial_corrected")

    except Exception as e:
        dbt.print_status(f"Error creating spatial data: {e}")
        dbt.print_status("No geographic data available")
...
try:
    ...
    dbt.print_status(f"Loaded {len(county_data)} counties with geographic and risk data")

    if len(county_data) > 0:
        # Data validation and cleaning
        dbt.print_status("Data validation:")
        dbt.print_status(
        ...
        dbt.print_status(f"NULL values: {county_data.isnull().sum().sum()}")
    else:
        dbt.print_status("No geographic data available - creating empty DataFrame")
        county_data = pd.DataFrame()
except Exception as e:
    dbt.print_status(f"Error loading geographic data: {e}")
    county_data = pd.DataFrame()
...
dbt.print_status("Loading depot analysis tables to database...")
dbt.load_data(depot_df, "strategic_depot_locations", if_exists="replace")
dbt.print_status("Data loaded successfully into strategic_depot_locations")

dbt.load_data(county_assignments, "county_depot_assignments", if_exists="replace")
dbt.print_status("Data loaded successfully into county_depot_assignments")

dbt.load_data(service_stats_df, "depot_service_statistics", if_exists="replace")
dbt.print_status("Data loaded successfully into depot_service_statistics")
...
for table in table_names["table_name"]:
    dbt.print_status(f"  ✓ {table}")

dbt.print_section_start(
    "ETL PIPELINE COMPLETE", "All data processing and analysis completed successfully"
)
timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
dbt.print_status(f"[{timestamp}] ETL pipeline execution finished.")
dbt.print_status(f"{'=' * 60}")
