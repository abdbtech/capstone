...
if __name__ == "__main__":
    ...
    dbt.print_status("Filtering files for 1999-2024 period...")
    selected_files = []

    for file in tqdm.tqdm(files, desc="Filtering FTP files"):
    ...
    dbt.print_status(f"Selected {len(selected_files)} StormEvents_details files for processing")
    for i, filename in enumerate(selected_files, 1):
        year = re.search(r"d(\d{4})", filename).group(1)
        print(f"  {i:2d}. {year}: {filename}")
...
# Check if table exists.
    ...
    if table_exists:
        ...
        for i, filename in enumerate(tqdm.tqdm(selected_files, desc="Downloading NOAA files"), 1):
            try:
                # Construct full URL
